...
QPaintsWidget::QPaintsWidget(QWidget *parent) : QWidget(parent)
{
        ratio =0.5;
}

void QPaintsWidget::setImage(const QImage& image,const QRect& paint)
{
    img =image;
...
void QPaintsWidget::paintEvent(QPaintEvent*)
{
...
void QPaintsWidget::mousePressEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
    PreDot = event->pos();
    m_bLeftPressed = true;
...
void QPaintsWidget::mouseReleaseEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
...
void QPaintsWidget::mouseMoveEvent(QMouseEvent *event)
{
    if (m_bLeftPressed) {
...
void QPaintsWidget::wheelEvent(QWheelEvent *event)
{
    if (event->delta()>0)
    {
        ratio-=0.05*ratio;
